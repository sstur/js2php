...
testSuite('json', function(assert) {
  ...
  testSuite("`parse`: Empty Source Strings", function() {
    ...
    testSuite("Source string containing multiple space characters", function() {
      parseError(" ", "");
    });
  });
  ...
  testSuite("`parse`: Whitespace", function() {
    ...
    testSuite("Source string containing an invalid Unicode whitespace character", function() {
    ...
    testSuite("Source string containing multiple line terminators", function() {
      parses({}, "{\n\n\r\n}");
    });
  ...
  testSuite("`parse`: String Literals", function() {
    ...
    testSuite("String containing an escaped Unicode line separator", function() {
      parses("\u2028", '"\\u2028"');
    });
    testSuite("String containing an escaped Unicode paragraph separator", function() {
      parses("\u2029", '"\\u2029"');
    });
    ...
    //  parses("\ud834\udf06", '"\ud834\udf06"');
    //});
    testSuite("String containing an escaped ASCII control character", function() {
      parses("\u0001", '"\\u0001"');
    });
    ...
    testSuite("String containing an escaped double-quote character", function() {
      parses("hello\"world", '"hello\\"world"');
    });
    ...
    testSuite("throw if containing an unescaped ASCII control character", function() {
  ...
  testSuite("`stringify` and `parse`: Optional Arguments", function() {
    ...
    serializes("{\n  \"bar\": 456\n}", {"bar": 456}, "Object; optional `whitespace` arguments", null, 2);
  ...
  testSuite("`stringify`", function() {
    ...
    testSuite("Nested objects containing identically-named properties should serialize correctly", function() {
      serializes('{"foo":{"b":{"foo":{"c":{"foo":null}}}}}', value);
    });

    var S = [], N = {};
    S.push(N, N);
    testSuite("Objects containing duplicate references should not throw a `TypeError`", function() {
      serializes('[{},{}]', S);
    });
    ...
    // Dates.
    testSuite("Dates should be serialized according to the simplified date time string format", function() {
      serializes('"1994-07-03T00:00:00.000Z"', new Date(Date.UTC(1994, 6, 3)));
    });
    testSuite("The date time string should conform to the format outlined in the spec", function() {
      serializes('"1993-06-02T02:10:28.224Z"', new Date(Date.UTC(1993, 5, 2, 2, 10, 28, 224)));
    });
    testSuite("The minimum valid date value should serialize correctly", function() {
      serializes('"-271821-04-20T00:00:00.000Z"', new Date(-8.64e15));
    });
    testSuite("The maximum valid date value should serialize correctly", function() {
      serializes('"275760-09-13T00:00:00.000Z"', new Date(8.64e15));
    });
    testSuite("https://bugs.ecmascript.org/show_bug.cgi?id=119", function() {
      serializes('"10000-01-01T00:00:00.000Z"', new Date(Date.UTC(10000, 0, 1)));
    });

    // Tests based on research by @Yaffle. See kriskowal/es5-shim#111.
    testSuite("Millisecond values < 1000 should be serialized correctly", function() {
      serializes('"1969-12-31T23:59:59.999Z"', new Date(-1));
    });
    testSuite("Years prior to 0 should be serialized as extended years", function() {
      serializes('"-0001-01-01T00:00:00.000Z"', new Date(-621987552e5));
    });
    testSuite("Years after 9999 should be serialized as extended years", function() {
      serializes('"10000-01-01T00:00:00.000Z"', new Date(2534023008e5));
    });
    testSuite("Issue #4: Opera > 9.64 should correctly serialize a date with a year of `-109252`", function() {
  ...
  testSuite("ECMAScript 5 Conformance", function() {
    ...
    testSuite("Valid whitespace characters may not separate two discrete tokens", function() {
      parseError("12\t\r\n 34");
    });
    testSuite("The vertical tab is not a valid whitespace character", function() {
      parseError("\u000b1234");
    });
    testSuite("The form feed is not a valid whitespace character", function() {
      parseError("\u000c1234");
    });
    testSuite("The non-breaking space is not a valid whitespace character", function() {
      parseError("\u00a01234");
    });
    testSuite("The zero-width space is not a valid whitespace character", function() {
      parseError("\u200b1234");
    });
    testSuite("The byte order mark (zero-width non-breaking space) is not a valid whitespace character", function() {
      parseError("\ufeff1234");
    });
    testSuite("Other Unicode category `Z` characters are not valid whitespace characters", function() {
      parseError("\u1680\u180e\u2000\u2001\u2002\u2003\u2004\u2005\u2006\u2007\u2008\u2009\u200a\u202f\u205f\u30001234");
    });
    testSuite("The line (U+2028) and paragraph (U+2029) separators are not valid whitespace characters", function() {
      parseError("\u2028\u20291234");
    });

    // Test 15.12.1.1-0-9.
    testSuite("Valid whitespace characters may precede and follow all tokens", function() {
    ...
    testSuite("A tab character may not separate two disparate tokens", function() {
      parseError("12\t34");
    });
    testSuite("Leading carriage returns should be ignored", function() {
      parses(1234, "\r1234");
    });
    testSuite("A carriage return may not separate two disparate tokens", function() {
      parseError("12\r34");
    });
    ...
    testSuite("A space character may not separate two disparate tokens", function() {
      parseError("12 34");
    });
    ...
    testSuite("Unicode-escaped double quote delimiters are not permitted", function() {
      parseError("\\u0022abc\\u0022");
    });
    testSuite("Strings must terminate with a double quote character", function() {
      parseError('"ab'+"c'");
    });
    ...
    testSuite("Unescaped control characters in the range [U+0000, U+0007] are not permitted within strings", function() {
      parseError('"\u0000\u0001\u0002\u0003\u0004\u0005\u0006\u0007"');
    });
    testSuite("Unescaped control characters in the range [U+0008, U+000F] are not permitted within strings", function() {
      parseError('"\u0008\u0009\u000a\u000b\u000c\u000d\u000e\u000f"');
    });
    testSuite("Unescaped control characters in the range [U+0010, U+0017] are not permitted within strings", function() {
      parseError('"\u0010\u0011\u0012\u0013\u0014\u0015\u0016\u0017"');
    });
    testSuite("Unescaped control characters in the range [U+0018, U+001F] are not permitted within strings", function() {
      parseError('"\u0018\u0019\u001a\u001b\u001c\u001d\u001e\u001f"');
    });

    // Tests 15.12.1.1-g5-1 thru 15.12.1.1-g5-3.
    testSuite("Unicode escape sequences are permitted within strings", function() {
      parses("X", '"\\u0058"');
    });
    testSuite("Unicode escape sequences may not comprise fewer than four hexdigits", function() {
      parseError('"\\u005"');
    });
    testSuite("Unicode escape sequences may not contain non-hex characters", function() {
      parseError('"\\u0X50"');
    });
    ...
    testSuite("`JSON.stringify(undefined)` should return `undefined`", function() {
      serializes(void 0, void 0);
    });

    testSuite("The `JSON.stringify` callback function can be called on a top-level `undefined` value", function() {
      serializes('"replacement"', void 0, "", function() {
        return "replacement";
      });
    });
    testSuite("`JSON.stringify` should serialize top-level string primitives", function() {
      serializes('"a string"', "a string");
    });
    testSuite("`JSON.stringify` should serialize top-level number primitives", function() {
      serializes("123", 123);
    });
    testSuite("`JSON.stringify` should serialize top-level Boolean primitives", function() {
      serializes("true", true);
    });
    testSuite("`JSON.stringify` should serialize top-level `null` values", function() {
      serializes("null", null);
    });
    testSuite("`JSON.stringify` should serialize top-level number objects", function() {
      serializes("42", new Number(42));
    });
    testSuite("`JSON.stringify` should serialize top-level string objects", function() {
      serializes('"wrapped"', new String("wrapped"));
    });
    testSuite("`JSON.stringify` should serialize top-level Boolean objects", function() {
      serializes("false", new Boolean(false));
    });
    testSuite("The `JSON.stringify` callback function may return `undefined` when called on a top-level number primitive", function() {
      serializes(void 0, 42, "", function() {
        return void 0;
      });
    });
    testSuite("The `JSON.stringify` callback function may return `undefined` when called on a top-level object", function() {
      serializes(void 0, { "prop": 1 }, "", function() {
        return void 0;
      });
    });
    testSuite("The `JSON.stringify` callback function may return an array when called on a top-level number primitive", function() {
      serializes("[4,2]", 42, "", function(key, value) {
        return value == 42 ? [4, 2] : value;
      });
    });
    testSuite("The `JSON.stringify` callback function may return an object literal when called on a top-level number primitive", function() {
      serializes('{"forty":2}', 42, "", function(key, value) {
        return value == 42 ? { "forty": 2 } : value;
      });
    });
    testSuite("`JSON.stringify` should return `undefined` when called on a top-level function", function() {
      serializes(void 0, function() {});
    });
    testSuite("The `JSON.stringify` callback function may return a number primitive when called on a top-level function", function() {
    ...
    testSuite("Optional `width` argument: The maximum numeric width value should be 10", function() {
      equals(JSON.stringify(value, null, 10), JSON.stringify(value, null, 100));
    });
    testSuite("Optional `width` argument: Numeric values should be converted to integers", function() {
      equals(JSON.stringify(value, null, 5.99999), JSON.stringify(value, null, 5));
    });

    // Test 15.12.3-6-b-1 and 15.12.3-6-b-4.
    testSuite("Optional `width` argument: Numeric width values between 0 and 1 should be ignored", function() {
    ...
    testSuite("Optional `width` argument: Negative numeric values should be ignored", function() {
      equals(JSON.stringify(value, null, -5), JSON.stringify(value));
    });
    testSuite("Optional `width` argument: Numeric width values in the range [1, 10] should produce identical results to that of string values containing `width` spaces", function() {
      equals(JSON.stringify(value, null, 5), JSON.stringify(value, null, "     "));
    });

    // Test 15.12.3-7-a-1.
    testSuite("Optional `width` argument: String width values longer than 10 characters should be truncated", function() {
    ...
    testSuite("Boolean primitive `width` arguments should be ignored", function() {
    ...
    testSuite("Boolean object `width` arguments should be ignored", function() {
      equals(JSON.stringify(value, null, new Boolean(false)), JSON.stringify(value));
    });
    testSuite("Object literal `width` arguments should be ignored", function() {
      equals(JSON.stringify(value, null, value), JSON.stringify(value));
    });

    // Test 15.12.3@2-2-b-i-1.
    testSuite("An object literal with a custom `toJSON` method nested within an array may return a string primitive for serialization", function() {
