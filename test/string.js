...
testSuite('strings', function (assert) {
  ...
  testSuite('charCodeAt', function () {
    ...
    assert('out of bound index', Number.isNaN('abc'.charCodeAt(3)));
  });
  ...
  testSuite('replace', function () {
    ...
    assert('regex insensitive global', s.replace(/cd/gi, '__') === 'ab__ab__');
    assert('regex capture', s.replace(/(c)(.)/gi, '$1_') === 'abc_abC_');
    assert('regex capture 2', s.replace(/(c)(..)/gi, '$2') === 'abdabCd');
    var fn = function () {
    ...
    assert(
      'regex fn insensitive global',
      s.replace(/cd/gi, fn) === 'ab[cd,2]ab[Cd,6]'
    );
    s = 'sstür';
  ...
  testSuite('search', function () {
    assert('simple', 'abcd'.search(/c/) === 2);
    var i = '↗Զ--x'.search(/-(\w)/);
    assert('unicode', i === 3);
  });
  ...
  testSuite('encode/decode uri functions', function () {
    ...
    assert(
      "unescape('%FC%u2197%u0536')",
      unescape('%FC%u2197%u0536') === 'ü↗Զ'
    );
    ...
    assert(
      "encodeURIComponent('↗Զ')",
      encodeURIComponent('↗Զ') === '%E2%86%97%D4%B6'
    );
    assert(
      "decodeURIComponent('%E2%86%97%D4%B6')",
      decodeURIComponent('%E2%86%97%D4%B6') === '↗Զ'
    );
  });

  testSuite('startsWith', function () {
    assert("'abcde' starts with 'a'", 'abcde'.startsWith('a') === true);
    assert("'abcde' starts not with 'c'", 'abcde'.startsWith('c') === false);
  });
});
