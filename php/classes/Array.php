...
Arr::$protoMethods = [
    'push' => function ($value) {
        ...
        $length = call_user_func_array([$self, 'push'], func_get_args());
        return (float) $length;
    },
    'pop' => function () {
        $self = Func::getContext();
    ...
    'unshift' => function ($value) {
        $self = Func::getContext();
        //for this we have a low-level method
        $length = call_user_func_array([$self, 'unshift'], func_get_args());
        return (float) $length;
    },
    'shift' => function () {
        $self = Func::getContext();
    ...
    'join' => function ($str = ',') {
        ...
        for ($i = 0; $i < $len; $i++) {
            $value = $self->get($i);
            $results[] =
                $value === null || $value === ObjectClass::$null
                    ? ''
                    : to_string($value);
        }
