...
class Func extends ObjectClass
{
    ...
    function apply($context, $args)
    {
        ...
        if (!$this->strict) {
            ...
            } elseif (!($context instanceof ObjectClass)) {
...
Func::$protoMethods = [
    ...
    'apply' => function ($context, $args = null) {
        ...
        if ($args === null) {
            $args = [];
        } elseif ($args instanceof Args || $args instanceof Arr) {
            $args = $args->toArray();
        } else {
            throw new Ex(
                Err::create(
    ...
    'toString' => function () {
        $self = Func::getContext();
        $source = array_key_exists('source_', $GLOBALS)
            ? $GLOBALS['source_']
            : null;
        if ($source) {
            $meta = $self->meta;
...
ObjectClass::$protoObject->setMethods(
