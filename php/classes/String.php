...
Str::$protoMethods = [
    ...
    'match' => function ($regex) use (&$RegExp) {
        ...
        while (
            preg_match($preg, $str, $matches, PREG_OFFSET_CAPTURE, $index) === 1
        ) {
            $foundAt = $matches[0][1];
        ...
        if ($replace instanceof Func) {
            if ($isRegEx) {
                ...
                if ($success === false) {
                    ...
                    throw new Ex(
                        Err::create('String.prototype.replace() failed')
                    );
                }
                ...
                $args = [$search, mb_strlen($before), $str];
                return $before .
                    to_string($replace->apply(null, $args)) .
                    $after;
            }
        }
