...
class ObjectClass
{
    ...
    function set($key, $value)
    {
        ...
        if (
            !array_key_exists($key, $this->dscr) ||
            $this->dscr[$key]->writable
        ) {
            $this->data[$key] = $value;
        }
        return $value;
...
ObjectClass::$classMethods = [
    ...
    'create' => function ($proto) {
        if (!($proto instanceof ObjectClass) && $proto !== ObjectClass::$null) {
            throw new Ex(
                Err::create('Object prototype may only be an Object or null')
            );
        }
        $obj = new ObjectClass();
        $obj->proto = $proto;
    ...
    'getOwnPropertyNames' => function ($obj) {
        if (!($obj instanceof ObjectClass)) {
            throw new Ex(
                Err::create('Object.getOwnPropertyNames called on non-object')
            );
        }
    ...
    'defineProperty' => function ($obj, $key, $desc) {
        ...
        if (!($obj instanceof ObjectClass)) {
            throw new Ex(
                Err::create('Object.defineProperty called on non-object')
            );
        }
        ...
        if (array_key_exists($key, $obj->data)) {
            ...
            if (!$result->configurable) {
                throw new Ex(
                    TypeErr::create('Cannot redefine property: ' . $key)
                );
            }
            if ($writable !== null) {
                $result->writable = !!$writable;
    ...
    'defineProperties' => function ($obj, $items) {
        if (!($obj instanceof ObjectClass)) {
            throw new Ex(
                Err::create('Object.defineProperties called on non-object')
            );
        }
        if (!($items instanceof ObjectClass)) {
            throw new Ex(
                Err::create(
                    'Object.defineProperties called with invalid list of properties'
                )
            );
        }
