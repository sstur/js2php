...
class Buffer extends ObjectClass
{
    ...
    function init($args)
    {
        ...
        } elseif (_instanceof($subject, $Buffer)) {
            $this->raw = $subject->raw;
        } elseif ($subject instanceof Arr) {
    ...
    'concat' => function (/*Arr*/ $list, $totalLength = null) {
        ...
        foreach ($list->toArray() as $buffer) {
            if (!($buffer instanceof Buffer)) {
                throw new Ex(
                    Err::create('Usage: Buffer.concat(array, [length])')
                );
            }
            $rawList[] = $buffer->raw;
...
Buffer::$protoMethods = [
    ...
    'write' => function ($data /*, $offset = null, $len = null, $enc = null*/) {
        ...
        if ($count > 0) {
            ...
            } elseif (is_int_or_float($args[0])) {
                $offset = array_shift($args);
                $enc = array_pop($args);
                $len = array_pop($args);
                //swap len/enc if necessary
                if (is_int_or_float($enc)) {
                    list($len, $enc) = [$enc, $len];
                }
            }
        }
