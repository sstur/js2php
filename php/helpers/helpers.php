...
{
    $typeA =
        $a === null || $a === ObjectClass::$null
            ? 'null'
            : ($a instanceof ObjectClass
                ? 'object'
                : gettype($a));
    $typeB =
        $b === null || $b === ObjectClass::$null
            ? 'null'
            : ($b instanceof ObjectClass
                ? 'object'
                : gettype($b));
...
function _bitwise_zfrs($a, $b)
{
    ...
    if ($b == 0) {
        return (float) ((($a >> 1) & 0x7fffffff) * 2 + (($a >> $b) & 1));
    }
...
function to_string($value)
{
    ...
    if ($value instanceof ObjectClass) {
        ...
        if ($fn instanceof Func) {
            ...
            throw new Ex(
                Err::create('Cannot convert object to primitive value')
            );
        }
    }
    throw new Ex(
        Err::create('Cannot cast PHP value to string: ' . _stringify($value))
    );
}
...
function to_number($value)
{
    ...
    if (
        preg_match('/^([+-]?)(\d+\.\d*|\.\d+|\d+)e([+-]?[0-9]+)$/i', $value, $m)
    ) {
...
function get($obj, $name)
{
    if ($obj === null || $obj === ObjectClass::$null) {
        throw new Ex(
            Err::create(
                "Cannot read property '" . $name . "' of " . to_string($obj)
            )
        );
    }
    $obj = objectify($obj);
...
function set($obj, $name, $value, $op = '=', $returnOld = false)
{
    if ($obj === null || $obj === ObjectClass::$null) {
        throw new Ex(
            Err::create(
                "Cannot set property '" . $name . "' of " . to_string($obj)
            )
        );
    }
    $obj = objectify($obj);
