...
Debug::$inspect = call_user_func(function () use (
    &$Date,
    &$Object,
    &$RegExp,
    &$JSON,
    ...
    $inspect = new Func('inspect', function ($obj = null, $depth = null) use (
        &$stylizeNoColor,
        &$formatValue
    ) {
        ...
        return call(
            ...
            $ctx,
            $obj,
            (isset($depth) ? _typeof($depth) : 'undefined') === 'undefined'
                ? 2.0
                : $depth
        );
    });
    $stylizeNoColor = new Func('stylizeNoColor', function (
        $str = null,
        $styleType = null
    ) {
        return $str;
    });
    $formatValue = new Func('formatValue', function (
        $ctx = null,
        $value = null,
        $recurseTimes = null
    ) use (
        ...
        if (
            is($value) &&
            _typeof(get($value, 'inspect')) === 'function' &&
            get($value, 'inspect') !== $inspect &&
            not(
                is($and_ = get($value, 'constructor'))
                    ? get(get($value, 'constructor'), 'prototype') === $value
                    : $and_
            )
        ) {
        ...
        if (is(call($isDate, $value))) {
            return call_method(
                ...
                'stylize',
                call_method(
                    get(get($Date, 'prototype'), 'toUTCString'),
                    'call',
                    $value
                ),
                'date'
        ...
        if (
            (isset($value) ? _typeof($value) : 'undefined') === 'object' &&
            !_instanceof($value, $Object)
        ) {
        ...
        if (get($keys, 'length') === 0.0) {
            if (
                (isset($value) ? _typeof($value) : 'undefined') === 'function'
            ) {
                $name = is(get($value, 'name'))
                    ? _concat(': ', get($value, 'name'))
                    : '';
                return call_method(
                    $ctx,
                    'stylize',
                    _concat('[Function', $name, ']'),
                    'special'
                );
            }
            if (is(call($isRegExp, $value))) {
                return call_method(
                    ...
                    'stylize',
                    call_method(
                        get(get($RegExp, 'prototype'), 'toString'),
                        'call',
                        $value
                    ),
                    'regexp'
        ...
        if (is($array)) {
            ...
            $output = call_method(
                $keys,
                'map',
                new Func(function ($key = null) use (
                    &$formatProperty,
                    &$ctx,
                    &$value,
                    &$recurseTimes,
    ...
    $formatPrimitive = new Func('formatPrimitive', function (
        $ctx = null,
        $value = null
    ) use (&$JSON) {
            ...
            case 'string':
                $simple = _concat(
                    ...
                    call_method(
                        call_method(
                            call_method(
                                call_method($JSON, 'stringify', $value),
                                'replace',
                                new RegExp("^\"|\"\$", 'g'),
                                ''
                            ),
    ...
    $formatError = new Func('formatError', function ($value = null) use (
        &$Error
    ) {
        return _concat(
            '[',
            call_method(
                get(get($Error, 'prototype'), 'toString'),
                'call',
                $value
            ),
            ']'
    ...
    $formatArray = new Func('formatArray', function (
        $ctx = null,
        $value = null,
        $recurseTimes = null,
        $keys = null
    ) use (&$Object, &$String, &$formatProperty) {
        $output = new Arr();
        for ($i = 0.0, $l = get($value, 'length'); $i < $l; ++$i) {
            if (
                is(
                    call_method(
                        get(get($Object, 'prototype'), 'hasOwnProperty'),
                        'call',
                        $value,
                        call($String, $i)
                    )
        ...
        call_method(
            ...
            new Func(function ($key = null) use (
                ...
                if (
                    not(call_method($key, 'match', new RegExp("^\\d+\$", '')))
                ) {
                    call_method(
                        $output,
                        'push',
    ...
    $formatProperty = new Func('formatProperty', function (
        $ctx = null,
        $value = null,
        $recurseTimes = null,
        $keys = null,
            ...
            if (call_method($str, 'indexOf', "\n") > -1.0) {
                if (is($array)) {
                    $str = call_method(
                        call_method(
                            call_method(
                                call_method($str, 'split', "\n"),
                                'map',
                                new Func(function ($line = null) {
                                    return _concat('  ', $line);
                                })
                            ),
                            'join',
        ...
        if ((isset($name) ? _typeof($name) : 'undefined') === 'undefined') {
            if (
                is($array) &&
                is(call_method($key, 'match', new RegExp("^\\d+\$", '')))
            ) {
                return $str;
            }
    ...
    $reduceToSingleString = new Func('reduceToSingleString', function (
        $output = null,
        $base = null,
        $braces = null
    ) {
        ...
        call_method(
            $output,
            'forEach',
            new Func(function ($str = null) use (&$numLinesEst, &$length) {
                $numLinesEst++;
                if (call_method($str, 'indexOf', "\n") >= 0.0) {
                    $numLinesEst++;
                }
    ...
    $isArray = new Func('isArray', function ($ar = null) use (
        &$Array,
        &$objectToString
    ) {
        return is($or_ = call_method($Array, 'isArray', $ar))
            ? $or_
            : (isset($ar) ? _typeof($ar) : 'undefined') === 'object' &&
                    call($objectToString, $ar) === '[object Array]';
    });
    $isRegExp = new Func('isRegExp', function ($re = null) use (
        &$objectToString
    ) {
        return (isset($re) ? _typeof($re) : 'undefined') === 'object' &&
            call($objectToString, $re) === '[object RegExp]';
    });
    $isDate = new Func('isDate', function ($d = null) use (&$objectToString) {
        return (isset($d) ? _typeof($d) : 'undefined') === 'object' &&
            call($objectToString, $d) === '[object Date]';
    });
    $isError = new Func('isError', function ($e = null) use (&$objectToString) {
        return (isset($e) ? _typeof($e) : 'undefined') === 'object' &&
            call($objectToString, $e) === '[object Error]';
    });
    $objectToString = new Func('objectToString', function ($o = null) use (
        &$Object
    ) {
        return call_method(
            get(get($Object, 'prototype'), 'toString'),
            'call',
            $o
        );
    });
