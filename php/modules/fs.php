...
Module::define('fs', function () {
    ...
    $ReadStream = new Func('ReadStream', function ($path, $opts = null) use (
        &$helpers,
        &$CHUNK_SIZE
    ) {
    ...
    $WriteStream = new Func('WriteStream', function ($path, $opts = null) use (
        &$helpers
    ) {
        $self = Func::getContext();
        $fullPath = $helpers['mapPath']($path);
    ...
    $methods = [
        ...
        'copyFile' => function ($src, $dst) use (&$helpers) {
            ...
            //fallback for if set_error_handler didn't do it's thing
            if ($result === false) {
                $helpers['throwError']('EIO', [$src, $dst]);
            }
        },
        ...
        'removeDirIfExists' => function ($path, $deep = false) use (
            &$helpers,
            &$fs
        ) {
            $fullPath = $helpers['mapPath']($path);
        ...
        'createReadStream' => function ($path, $opts = null) use (
            &$helpers,
            &$ReadStream
        ) {
            return $ReadStream->construct($path, $opts);
        },
        'createWriteStream' => function ($path, $opts = null) use (
            &$helpers,
            &$WriteStream
        ) {
    ...
    $helpers = [
        ...
        'throwError' => function ($code, $paths = [], $framesToPop = 0) use (
            &$helpers
        ) {
        ...
        'handleException' => function ($ex, $paths = []) use (&$helpers) {
            ...
            } elseif (strpos($message, 'stat failed for') !== false) {
                $helpers['throwError']('ENOENT', $paths, 1);
            } elseif (strpos($message, 'Directory not empty') !== false) {
                $helpers['throwError']('ENOTEMPTY', $paths, 1);
            } elseif (strpos($message, 'Is a directory') !== false) {
