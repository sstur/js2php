...
Test::suite('null', function () {
    Test::assert('is falsy', is(ObjectClass::$null) ? false : true);
    Test::assert('check typeof', _typeof(ObjectClass::$null) === 'object');
    Test::assert('check to_string', to_string(ObjectClass::$null) === 'null');
});

Test::suite('global', function () {
    Test::assert(
...
Test::suite('Object.prototype', function () use ($Object) {
    Test::assert(
        'has method valueOf',
    ...
    Test::assert(
        'check toString',
        $Object->get('prototype')->get('toString') ===
            ObjectClass::$protoObject->get('toString')
    );
});

Test::suite('keys()', function () use ($Object) {
...
Test::suite('Object', function () use ($Object) {
    $o = new ObjectClass('a', 1.0, 'b', 2.0);
    Test::assert('data present', join(',', array_keys($o->data)) === 'a,b');
    Test::assert(
        'descriptors not present',
        join(',', array_keys($o->dscr)) === ''
    );
    $o->setProp('c', 3.0, true, false, true);
    Test::assert('three keys', join(',', array_keys($o->data)) === 'a,b,c');
    Test::assert(
        'descriptors present',
    ...
    Test::assert('keys now two', join(',', array_keys($o->data)) === 'a,b');
    $o = new ObjectClass('a', 1.0, 'b', 2.0);
    Test::assert('for..in helper', join(',', keys($o)) === 'a,b');
});

Test::suite('Array.prototype', function () use ($Object, $Array) {
    Test::assert(
...
Test::suite('Array: init, push and join', function () use ($Array) {
    $arr = $Array->construct();
    $arr->set(0.0, 'a');
    Test::assert('proto exists', $arr->proto instanceof ObjectClass);
    Test::assert(
        'proto is set correctly',
        $arr->proto === $Array->get('prototype')
    );
    Test::assert(
        'proto chain',
        $arr->proto->proto === ObjectClass::$protoObject
    );
...
Test::suite('Array: set length', function () use ($Array) {
...
Test::suite('Array: sort', function () use ($Array) {
...
Test::suite('Function', function () use ($Array) {
    $fn = new Func('foo', function () {
        return 'bar';
    });
    Test::assert(
        'Function should take optional name',
        $fn->get('name') === 'foo'
    );
    $fn->set('name', 'x');
...
Test::suite('Object.create', function () use ($Object) {
    ...
    $Dog->set(
        'prototype',
        $Object->callMethod('create', $Animal->get('prototype'))
    );
    $dog = $Dog->construct();
    ...
    Test::assert(
        'method still on parent',
        $animal->callMethod('speak') === 'hi'
    );
});

Test::suite('Object.keys', function () use ($Object, $Array, $JSON) {
    $obj = $Object->construct();
    $obj->set('a', 1.0);
    $obj->set('b', 2.0);
    ...
    Test::assert(
        ...
        $keys->callMethod('toString') === '0,1'
    );
});

Test::suite('Object.defineProperty', function () use ($Object) {
    $obj = $Object->construct();
    $obj->set('a', 1.0);
...
Test::suite('Date', function () use ($Date, $Object, $Array) {
    ...
    Test::assert(
        'date valueOf',
        (float) $date->value === $date->callMethod('valueOf')
    );
    Test::assert(
        'date local string',
        $date->callMethod('toString') ===
            'Mon Aug 05 2013 18:11:08 GMT-0700 (MST)'
    );
    Test::assert(
        'date json string',
        $date->callMethod('toJSON') === '2013-08-06T01:11:08.411Z'
    );
    $date = new Date(1375751468412.0);
    Test::assert(
        'init from value',
        $date->callMethod('toJSON') === '2013-08-06T01:11:08.412Z'
    );
...
Test::suite('String object', function () use ($String, $Object) {
    ...
    Test::assert(
        'can call on primitive',
        call_method($str, 'charAt', 0) === 'h'
    );
});

Test::suite('Number helpers', function () use ($Number) {
    ...
    Test::assert(
        'to_number "+1234.5678"',
        to_number('+1234.5678') === to_number('1234.5678')
    );
    Test::assert(
        'to_number "+1234.5678e90"',
        to_number('+1234.5678e90') === to_number('1234.5678e90')
    );
    Test::assert(
        'to_number "+1234.5678E90"',
        to_number('+1234.5678E90') === to_number('1234.5678E90')
    ...
    Test::assert(
        'to_number "+1234.5678e-90"',
        to_number('+1234.5678e-90') === to_number('1234.5678e-90')
    );
    Test::assert(
        'to_number "+1234.5678E-90"',
        to_number('+1234.5678E-90') === to_number('1234.5678E-90')
...
Test::suite('JSON', function () use ($JSON, $Object, $Array) {
    $str = '{"a": [1], "b": false, "c": []}';
